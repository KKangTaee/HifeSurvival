...
namespace UniRx
{
    ...
    public static partial class Observable
    {
        ...
        public static IObservable<IList<T>> Reduce<T>(this IObservable<T> source, TimeSpan timeSpan)
        {
            return Reduce(source, timeSpan, Scheduler.DefaultSchedulers.TimeBasedOperations);
        }

        public static IObservable<IList<T>> Reduce<T>(this IObservable<T> source, TimeSpan timeSpan, IScheduler scheduler)
        {
            if (source == null) throw new ArgumentNullException("source");

            return new ReduceObservable<T>(source, timeSpan, scheduler);
        }

        public static IObservable<IList<TSource>> Reduce<TSource, TWindowBoundary>(this IObservable<TSource> source, IObservable<TWindowBoundary> windowBoundaries)
        {
            return new ReduceObservable<TSource, TWindowBoundary>(source, windowBoundaries);
        }
