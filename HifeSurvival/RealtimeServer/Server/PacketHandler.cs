...
namespace Server
{
    ...
    {
        public static new void CS_SelectHeroHandler(PacketSession session, IPacket packet)
        {
            CS_SelectHero selectHero = packet as CS_SelectHero;
            Push(session, room => { room?.Mode.OnRecvSelect(selectHero); });
        }

        public static new void C_JoinToGameHandler(PacketSession session, IPacket packet)
        {
        ...
        public static new void CS_AttackHandler(PacketSession session, IPacket packet)
        {
            CS_Attack attack = packet as CS_Attack;
            Push(session, room => room?.Mode.OnRecvAttack(attack));
        }

        public static new void CS_MoveHandler(PacketSession session, IPacket packet)
        {
            CS_Move move = packet as CS_Move;
            Push(session, room => room?.Mode.OnRecvMove(move));
        }

        public static new void CS_StopMoveHandler(PacketSession session, IPacket packet)
        {
            CS_StopMove stopMove = packet as CS_StopMove;
            Push(session, room => room?.Mode.OnRecvStopMove(stopMove));
        }

        public static new void CS_ReadyToGameHandler(PacketSession session, IPacket packet)
        {
            CS_ReadyToGame readyToGame = packet as CS_ReadyToGame;
            Push(session, room => room?.Mode.OnRecvReady(readyToGame));
        }

        public static new void CS_UpdateStatHandler(PacketSession session, IPacket packet)
        {
        ...
        internal static new void C_GetItemHandler(PacketSession session, IPacket packet)
        {
